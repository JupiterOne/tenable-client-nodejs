...
export function buildUserAgentHeader(options: { vendor: string, product: string, build: string }) {
  ...
  const tenableClientNodejsUserAgent = `@jupiterone/tenable-client-nodejs/${tenableClientNodejsBuildVersion} (@jupiterone/tenable-client-nodejs/${tenableClientNodejsBuildVersion}; Node.js/${process.version}; ${osPlatform()}/${osRelease})`;
...
export default class TenableClient {
  ...
  public async fetchUserPermissions() {
    return this.request<UserPermissionsResponse>(
      '/session',
      Method.GET,
    );
  }
  ...
  public async fetchScans() {
    return this.request<ScansResponse>(
      '/scans',
      Method.GET,
    );
  }
